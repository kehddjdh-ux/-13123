#!/usr/bin/env python3
# InnoDB recovery helper v3
# SYS_TABLES offsets calibrated byte-for-byte against the real MariaDB 10.11
# dump taken from pages-ibdata1 (record at 11063, mysql/innodb_table_stats):
#
#   NAME(var) | 8B | ID(8B) @+8 | 8B | N_COLS(4B) @+24 | TYPE(4B) @+28 |
#   MIX_ID(8B) @+32 | MIX_LEN(4B) @+40 | SPACE(4B) @+44 | next record @+48
#
# SYS_INDEXES layout is auto-detected per record: TABLE_ID/INDEX_ID are found
# in the 40 bytes before the index name, SPACE/PAGE_NO in the 40 bytes after,
# validated against the SPACE value that SYS_TABLES reported for that table.
#
# Modes:
#   python3 rec3.py map  [ibdata1] [name-filter]
#   python3 rec3.py inv  [pages-dir]
#   python3 rec3.py pull <space_id> <index_id> <out.page> [pages-dir]
import re
import sys
import glob
import os
    ...
    for m in re.finditer(rb"PRIMARY|GEN_CLUST_INDEX", b):
        ...
        for off in range(0, 25):
            ...
            for j in range(0, 33, 1):
                ...
                root = u(post, j + 4, 4)
                if 0 < root < 10 ** 7:
                    cands.append((tid, iid, want, root))
        if cands:
            tid, iid, sp, root = cands[0]
            found[tid].add((iid, sp, root))
    return found
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in ("map", "inv", "pull"):
        print("usage:")
        print("  rec3.py map [ibdata1] [filter]")
        print("  rec3.py inv [pages-dir]")
        print("  rec3.py pull <space> <index> <out.page> [pages-dir]")
        return 1
    mode = sys.argv[1]
    rest = sys.argv[2:]
    if mode == "map":
